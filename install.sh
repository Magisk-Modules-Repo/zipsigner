...
ui_print " ";
ui_print "ZipSigner Installer Script";
...
mkdir -p $bin;
cp -rf bin/* $bin;
chown 0:0 "$bin/zipsigner" "$bin/zipsigner"*.jar "$bin/zip" "$bin/zipalign";
chmod 755 "$bin/zipsigner" "$bin/zip" "$bin/zipalign";
chmod 644 "$bin/zipsigner"*.jar;
if [ "$magisk" ]; then
